...
const forgotPassword = async function (req, res, next) {
  ...
  if (user == null) {
    return res.json({ status: "fail", data: null, err: "No such user found" });
  } else {
    const { email } = user;
    ...
    return res.json({ status: "success", data: { token: token }, err: null });
  }
};
...
const reset_password = async function (req, res, next) {
  ...
  res.send({ status: "success", data: { redirectLink: "/login" }, err: null });
};
